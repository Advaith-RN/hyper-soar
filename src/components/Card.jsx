...
const Card = ({ name, photo, job,desc}) => {
  return (
    <div className='bg-white w-full md:w-[30%] lg:w-1/4 max-h-[75%] md:h-full p-4 group rounded-md hover:shadow-lg transition-shadow'>
        <p className='hidden group-hover:block w-full text-md font-inter p-2'>{desc}</p>
        <img src={photo} alt={name} className='w-full h-[75%] object-cover group-hover:opacity-0 '/>
