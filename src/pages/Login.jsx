  ...
  return (
    <div className="w-screen h-screen flex align-middle items-center justify-center">
      <div className="bg-[url('./assets/bg_2.jpg')] bg-cover bg-center w-full h-full x-[20%] absolute z-0 brightness-100"></div>
        <div className='group w-[90%] md:w-[40%] h-[75%] bg-dark-gray flex align-middle items-center flex-col gap-6 z-10 rounded-3xl shadow-lg shadow-zinc-800'>
          <img src={logo} alt="logo" className='invert mt-[8%] w-[25%] hover:bg-pink hover:invert-0 transition-all rounded-3xl' onClick={()=>{
            navigate('/');
          }}/>
          <input type='text' className='px-6 py-5 w-[60%] bg-white mt-[4%] h-8 font-inter outline-none focus:border-b-4 border-pink transition-all'/>
          ...
          {wrongPassword && <p className='text-md text-pink p-2 font-inter'>Your Username or Password is incorrect</p>}
        </div>
    </div>
  )
}
